...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.FastMode {
		other.SetPublic("fast_mode", true)
	}
	...
	if ctx != nil && ctx.Request != nil {
		if runtime := observability.FromContext(ctx.Request.Context()); runtime != nil {
			...
			runtime.EnrichLogOther(ctx.Request.Context(), enriched)
			if admin, ok := enriched["admin_info"].(map[string]interface{}); ok {
				other.MergeAdmin(admin)
			}
		}
	}
	return other
